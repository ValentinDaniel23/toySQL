...
TEST(StorageTest, multipleInternalKeys) {
    std :: string path = ROOT;
    path += "testBtree";
    path += OS_SEP;
    path += "mIK.txt";
...
TEST(StorageTest, fullBigTree) {
    ...
    for (char c = 'a'; c <= 'z'; c ++) {
        for (int i = 0; i < 2; i ++) {
            uint32_t id = static_cast<uint32_t>(c - 'a') * 2 + i;
            std :: unique_ptr<Row> row = std::make_unique<Row>(Row{id, 1, c});
